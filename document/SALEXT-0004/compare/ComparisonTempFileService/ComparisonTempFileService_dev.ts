...
export class ComparisonTempFileService {
  ...
  public async createOrUpdate(
    ...
    const safeOrg = this.sanitizeForFileName(orgAlias);
    // Visible name: MyClass.__from__UAT.cls — makes Org side obvious in tabs/diff.
    const fileName = `${parsed.name}.__from__${safeOrg}${parsed.ext}`;
   ...
   * @returns Label such as `Comparison Org: uat`.
   */
  public static formatOrgLabel(orgAlias: string): string {
    return `Comparison Org: ${orgAlias}`;
  }
