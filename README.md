...
Then in VS Code / Cursor: **Extensions → … → Install from VSIX…** and select the generated `.vsix` (`salesforce-compare-0.1.2.vsix`).
...
- **CompareService** — background queue → retrieve → cache → hash → status
